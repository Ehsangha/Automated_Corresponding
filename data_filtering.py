...
def filter_professor_data(professor_name, project_directory):
    ...
    # Filter articles since 2022
    current_year = datetime.now().year
    articles_filtered = [article for article in articles_unique if article['year'] >= 2022 and article['year'] <= current_year]
...
def extract_article_from_scholarly(pub, professor_name, logger):
    try:
        ...
        try:
            ...
            logger.warning(f"Invalid publication year '{pub_year_str}' for article '{title}'. Skipping.")
            return None
        ...
        if not professor_in_authors:
            logger.debug(f"Professor '{professor_name}' not in authors for article '{title}'. Skipping.")
            return None
...
def extract_article_from_crossref(pub, professor_name, logger):
    try:
        ...
        if year == 0:
            logger.warning(f"No valid publication year for article '{title}'. Skipping.")
            return None
...
def sort_articles(articles, professor_name):
    # First, articles where professor is first or corresponding author
    def article_sort_key(article):
        # Return a tuple: (is_professor_first_or_corr_author, year descending)
        is_prof_first_or_corr = article['professor_is_first_author'] or article['professor_is_corresponding_author']
        return (
            not is_prof_first_or_corr,  # Articles where professor is first or corr author come first (False < True)
            -article['year']  # Sort by year descending
        )
    articles_sorted = sorted(articles, key=article_sort_key)
...
def is_name_match(professor_name, author_name):
    ...
    # Match based on initials and last names
    if len(professor_name_parts) >= 2 and len(author_name_parts) >= 2:
        prof_initial = professor_name_parts[0][0]
        prof_last = professor_name_parts[-1]
        auth_initial = author_name_parts[0][0]
        auth_last = author_name_parts[-1]
        if prof_last == auth_last and prof_initial == auth_initial:
            return True
